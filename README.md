...
* **At the ceiling: Full Stop vs Crawl.** What the strokers do once your pulse crosses the pullback trigger. *Full Stop* parks the primary at 0%; *Crawl* keeps a 10% micro-motion so the edge stays alive. It applies in every mode that teases you down, including an Edge Training hold and every Oracle state reached while on the mark. The exception is *Survival Mode*: there the speed climbs on its own clock whatever your pulse does, and the run ends when you breach the working ceiling on three consecutive readings, so the strokers never park on the mark. Force Orgasm overrides both.
