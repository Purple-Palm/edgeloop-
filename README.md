...
The **Audio & Mic** tab has spoken voice guidance (local browser TTS, with a voice picker and Preview). When it is on, each cue is shown on the dashboard **and** spoken. Every event can hold many phrases (one per line; a random line is picked each time). Tokens `{hr}`, `{maxHr}`, `{minHr}`, `{edges}`, `{minutes}` fill in live session values. **Export phrases** / **Import phrases** save or load a JSON (or a `# edge` / `# encourage` text file); the full Backup export includes the same lists. Encouragement lines rotate on a timer during the session (default every 45 s, 0 = off).
